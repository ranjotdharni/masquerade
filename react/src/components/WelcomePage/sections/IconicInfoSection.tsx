...
export default function IconicInfoSection() {
    ...
    return (
        <section className="w-full flex flex-col items-center mt-16">
            ...
            <article className="flex flex-row space-x-10 mt-6">
                {
                    srcArray.map((detail, index) => {
                        return <img key={`ICONIC_INFO_TOOL_1x${index}`} src={detail.src} alt={detail.alt} className="h-16 aspect-square" />
                    })
                }
            </article>

            <article className="flex flex-row items-center justify-center space-x-14 mt-18">
                {
                    iconDetailsArray.map((detail, index) => {
                        return (
                            <figure key={`ICONIC_INFO_DETAIL_1x${index}`} className="flex flex-col justify-end items-center space-y-4">
                                <img src={detail.src} className="w-24 aspect-square bg-text p-4 rounded-[100px]" />
                                <h4 className="text-xl text-secondary font-jbm-bold">{detail.title}</h4>
                                <figcaption className="w-40 text-center text-center text-inactive font-roboto">{detail.text}</figcaption>
                            </figure>
                        )
                    })
                }
