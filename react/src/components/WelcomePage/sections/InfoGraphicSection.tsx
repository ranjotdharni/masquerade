    ...
    return (
        <li className={`w-full md:w-auto flex flex-col ${ reverse ? "md:flex-row-reverse" : "md:flex-row" }`}>
            ...
            <div className="flex flex-col justify-evenly items-center md:items-start text-left max-w-130 py-2 space-y-6 md:space-y-0">
                ...
                <p className="text-center md:text-left font-jbm text-secondary dark:text-inactive text-[1.1rem] px-3 md:px-0">{text}</p>
                <Link to={`/${PAGE_LOGIN}`} className="font-roboto-bold text-background dark:text-accent px-9 py-3 text-[1.4rem] bg-gradient-to-r from-primary to-text dark:from-secondary dark:to-secondary rounded-[100px] hover:cursor-pointer transition-transform hover:cursor-pointer hover:scale-105">{buttonText}</Link>
            </div>
        </li>
    )
}
    ...
    return (
        <section className="w-full mt-16">
            <header className="w-full flex flex-col items-center justify-center">
                <h3 className="text-[2rem] font-roboto-bold text-primary text-center dark:text-text">Keep Up With Your Audience Through {APP_NAME}</h3>
                <Link to={`${PAGE_USAGE}`} className="mt-4 text-[1.3rem] font-roboto-bold text-text dark:text-secondary underline hover:cursor-pointer">How it works</Link>
            </header>
